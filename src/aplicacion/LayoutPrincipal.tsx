    ...
    <div className="shell">
      <aside className="sidebar">
        ...
        <nav className="nav" aria-label="Navegación principal">
          ...
          {navegacion.map((item) => (
            <NavLink className={({ isActive }) => (isActive ? 'nav-item activo' : 'nav-item')} end={item.ruta === '/'} key={item.ruta} to={item.ruta}>
          ...
          <span className="nav-item nav-item-inactivo"><span className="nav-icono" aria-hidden="true">⚙</span>Configuración</span>
          <span className="nav-item nav-item-inactivo"><span className="nav-icono" aria-hidden="true">◉</span>Auditoría</span>
        </nav>

        <button className="usuario-sidebar" type="button" onClick={() => void salir()} aria-label="Cerrar sesión">
          <span className="avatar">{iniciales}</span>
          <span><strong>{usuario?.nombres} {usuario?.apellidos}</strong><small>Administrador · Salir</small></span>
        </button>
      </aside>

      <div className="area-principal">
        <header className="topbar">
          <a className="marca-mobile" href="/" aria-label="CICUC, ir al inicio"><span className="logo-punto" />CICUC</a>
          ...
          <div className="acciones-topbar">
            ...
            <button className="icon-btn" type="button" aria-label="Buscar">⌕</button>
            <button className="icon-btn" type="button" aria-label="Notificaciones">○<span className="alerta-punto" aria-hidden="true">▲</span></button>
            <button className="avatar salir-mobile" type="button" onClick={() => void salir()} aria-label="Cerrar sesión">{iniciales}</button>
          </div>
        </header>
        ...
        <nav className="nav-mobile" aria-label="Navegación principal">
          {navegacion.map((item) => (
            <NavLink className={({ isActive }) => (isActive ? 'nav-mobile-item activo' : 'nav-mobile-item')} end={item.ruta === '/'} key={item.ruta} to={item.ruta}>
              <span aria-hidden="true">{item.icono}</span><small>{item.etiqueta}</small>
            </NavLink>
          ))}
        </nav>
      </div>
