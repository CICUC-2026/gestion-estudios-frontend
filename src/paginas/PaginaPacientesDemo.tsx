...
export function PaginaPacientesDemo() {
  ...
  const [resultadoBiomarcador, setResultadoBiomarcador] = useState('')
  ...
  async function guardarDiagnostico(evento: React.FormEvent) {
    ...
    try {
      await crearDiagnosticoDemo(seleccionado.id, { diagnostico, biomarcador: biomarcador || undefined, resultado_biomarcador: resultadoBiomarcador || undefined, fecha: new Date().toISOString().slice(0, 10), fuente: 'Registro manual sintético' })
      setDiagnostico(''); setBiomarcador(''); setResultadoBiomarcador(''); setMensaje('Antecedente ficticio persistido y auditado.')
  ...
  async function cambiarEstadoPaciente(estadoNuevo: EstadoPacienteDemo) {
    if (!seleccionado) return
    const actualizado = await actualizarPacienteDemo(seleccionado.id, { estado: estadoNuevo })
    setSeleccionado(actualizado); await recargar(); setMensaje('Estado administrativo actualizado y auditado.')
  }
  ...
  return <>
    ...
    {seleccionado ? <section className="panel-vacio"><p className="sobrelinea">Gestión de {seleccionado.codigo}</p><h2>Antecedentes y estudios</h2><div className="formulario-paciente-demo"><label>Estado administrativo<select value={seleccionado.estado} onChange={(e) => void cambiarEstadoPaciente(e.target.value as EstadoPacienteDemo)}>{estados.map(([valor, etiqueta]) => <option value={valor} key={valor}>{etiqueta}</option>)}</select></label></div><form className="formulario-paciente-demo" onSubmit={(e) => void guardarDiagnostico(e)}><label>Diagnóstico ficticio<input required value={diagnostico} onChange={(e) => setDiagnostico(e.target.value)} /></label><label>Biomarcador ficticio<input value={biomarcador} onChange={(e) => setBiomarcador(e.target.value)} /></label><label>Resultado ficticio<input value={resultadoBiomarcador} onChange={(e) => setResultadoBiomarcador(e.target.value)} /></label><div className="acciones-formulario"><button className="boton-primario" type="submit">Guardar antecedente</button></div></form><div className="formulario-paciente-demo"><label>Estudio<select value={estudioId} onChange={(e) => setEstudioId(e.target.value)}>{estudios.map((item) => <option value={item.id} key={item.id}>{item.codigo_interno} · {item.titulo}</option>)}</select></label><div className="acciones-formulario"><button className="boton-primario" type="button" onClick={() => void asociar()}>Asociar para revisión</button></div></div></section> : null}
  </>
}
